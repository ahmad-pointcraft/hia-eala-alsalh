...
- [ ] CHK003 Are all black-based rgba opacity variants (0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.95) enumerated in the spec? [Completeness, Gap]
...
- [ ] CHK006 Is it specified whether token values are flat strings or nested objects by namespace? [Clarity, Gap]
...
- [ ] CHK018 Is the heaviest-migration file (EventModeDisplay.tsx, ~20 instances) called out for prioritized verification? [Coverage, Gap]
...
- [ ] CHK020 Are SVG gradient stop colors (used in EventModeDisplay.tsx, IslamicGeometricOverlay.tsx) covered by the migration strategy? [Completeness, Gap]
- [ ] CHK021 Is the strategy for constructing gradient strings that embed hex values (e.g., `linear-gradient(..., #D4AF37, ...)`) documented? [Clarity, Gap]
- [ ] CHK022 Does the spec address whether SVG elements inside MUI components can use theme-aware alternatives (e.g., CSS currentColor)? [Coverage, Gap]
...
- [ ] CHK024 Is the tool/method for detecting dead code specified (e.g., TypeScript compiler --noUnusedLocals, linter rule)? [Clarity, Gap]
...
- [ ] CHK028 Are requirements specified for what happens if a migrated component renders a slightly different shade due to rgba construction differences? [Edge Case, Gap]
...
- [ ] CHK030 Does the spec specify exact grep patterns or search queries for verifying zero hardcoded colors remain (SC-001, SC-002, SC-003)? [Clarity, Spec §SC-001–SC-003]
...
- [ ] CHK033 Are requirements for bundle size impact (regression or improvement) specified or explicitly out of scope? [Coverage, Gap]
...
- Items marked `[Gap]` indicate the spec does not currently address this aspect — these are requirements quality issues to resolve before planning.
