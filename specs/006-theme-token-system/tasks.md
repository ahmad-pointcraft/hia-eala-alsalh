...
- [ ] T005 [P] [US2] Migrate `src/app/components/FundraisingOverlay.tsx` (~10 color instances): (1) Add `import { colors } from "../theme/tokens"`. (2) Replace `rgba(0,0,0,0.4)` with `theme.palette.surface.raised` (Backdrop bgcolor). (3) Replace `rgba(0,0,0,0.6)` with `theme.palette.surface.deep` (Paper bgcolor). (4) Replace `rgba(212,175,55,0.3)` with `theme.palette.border.medium` (dividers, top border). (5) Replace `#D4AF37` and `#FFD700` in gradient strings with `${colors.gold.main}` and `${colors.gold.light}`. (6) Replace `bgcolor: "white"` with `bgcolor: "common.white"` (MUI built-in `theme.palette.common.white`). (7) Replace `color: "black"` with `colors.text.onGold` (token import for QR code text). (8) Run `yarn build`.
- [ ] T006 [P] [US2] Migrate `src/app/components/IslamicGeometricOverlay.tsx` (~14 color instances — SVG + inline styles): (1) Add `import { colors } from "../theme/tokens"`. (2) Replace all `stroke="#D4AF37"` SVG attributes with `stroke={colors.gold.main}` — **currentColor check**: if an SVG element is a child of an MUI component that sets color via sx, use `stroke="currentColor"` instead. Only use token imports for standalone SVGs. (3) Replace `#D4AF37` in gradient string interpolation with `${colors.gold.main}`. (4) Replace `#D4AF37` in particle backgroundColor with `colors.gold.main`. (5) Replace `rgba(212, 175, 55, 0.3)` in boxShadow keyframes with `colors.glow.subtle`. (6) Replace `rgba(212, 175, 55, 0.5)` in boxShadow keyframes with `colors.glow.medium`. (7) Replace `rgba(212, 175, 55, 0.3)` in radial gradient with `colors.border.medium`. (8) Run `yarn build`.
...
- [ ] T008 [P] [US2] Migrate `src/app/components/AnnouncementsTicker.tsx` (~4 color instances): (1) Replace `rgba(0,0,0,0.4)` with `theme.palette.surface.raised` (Paper bgcolor). (2) Replace `rgba(212,175,55,0.3)` with `theme.palette.border.medium` (border). (3) Replace `rgba(0,0,0,0.6)` with `theme.palette.surface.deep` (logo area bgcolor). (4) Replace inline `color: 'black'` on Megaphone icon with `colors.text.onDark` — this is an inline `style` attribute (not sx), so use token import: add `import { colors } from "../theme/tokens"` and set `style={{ color: colors.text.onDark }}`. (5) Run `yarn build`.
...
- [ ] T014 [US4] Dead code audit across all modified files: (1) Check each file modified in T003–T013 for unused imports — especially any old color-related imports that may no longer be needed. (2) Check if any components no longer need imports that were only used for hardcoded colors (e.g., if `alpha` was imported from MUI but is no longer used). (3) Run `yarn typecheck` (or `./node_modules/.bin/tsc --noUnusedLocals --noUnusedParameters`) and fix any reported issues. (4) Check `src/app/components/MasjidInfo.tsx` specifically for unused imports (flagged in original audit). (5) Run `yarn build` to confirm zero errors.
